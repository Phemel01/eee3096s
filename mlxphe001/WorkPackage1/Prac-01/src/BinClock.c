...
//                 int tes=wiringPiISR(BTNS[0],INT_EDGE_FALLING,&myInterrupts);
  //               printf("testing1.1 %d",tes);
                
                if (wiringPiISR(BTNS[0],INT_EDGE_FALLING,&myInterrupts)){
                  hourInc();
                  printf("test1");
}               
               if (wiringPiISR(BTNS[1],INT_EDGE_FALLING,&myInterrupt)){ 
                  minInc();
                   printf("test2");}
          //     int x= wiringPiISR (BTNS[0],INT_EDGE_FALLING, myInterrupts());
               // printf("here %d",digitalRead(BTNS[0]));
            //   printf("test 2 %d",x);
                //Toggle Seconds LED
		//Write your logic here
...
void hourInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		//Write hours back to the RTC
                ...
                hours=hexCompensation(wiringPiI2CReadReg8(RTC, HOUR_REGISTER)+1);
...
void minInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		//Write minutes back to the RTC
                mins=hexCompensation(wiringPiI2CReadReg8(RTC, MIN_REGISTER)+1);
                if(mins>59){
                 hourInc();
                 mins=0;
